...
OUTPUT_DIR = Path("vam_output")


def fetch_vam(query: str = QUERY, limit: int = LIMIT) -> List[Dict[str, Any]]:
    ...
    docs = raw.get("records", [])

    items = []

    for idx, doc in enumerate(docs, start=1):
        maker = doc.get("_primaryMaker") if isinstance(doc.get("_primaryMaker"), dict) else {}
        place = doc.get("_primaryPlace") if isinstance(doc.get("_primaryPlace"), dict) else doc.get("_primaryPlace")
        image_id = first_present(doc.get("_primaryImageId"), doc.get("primary_image_id"))

        image_url = None
        if is_present(image_id):
            image_url = f"https://framemark.vam.ac.uk/collections/{image_id}/full/!600,600/0/default.jpg"

        items.append({
            "position": idx,
            "object_id": doc.get("systemNumber"),
            "title": join_values(first_present(doc.get("_primaryTitle"), doc.get("title"))),
            "creator": join_values(first_present(maker.get("name"), doc.get("artistMakerPerson"))),
            "date": join_values(first_present(doc.get("_primaryDate"), doc.get("date"))),
            "place": join_values(first_present(place.get("name") if isinstance(place, dict) else place, doc.get("place"))),
            "object_type": join_values(first_present(doc.get("objectType"), doc.get("object_type"))),
            "image_url": image_url,
            "rights": join_values(first_present(doc.get("rights"), doc.get("copyright"))),
            "landing_page": f"https://collections.vam.ac.uk/item/{doc.get('systemNumber')}/" if doc.get("systemNumber") else None,
            "source_specific": {
                "accession_number": doc.get("accessionNumber"),
                "primary_image_id": image_id,
                "current_location": join_values(doc.get("_currentLocation")),
            },
        })
...
def main() -> None:
    print(f"Fetching V&A records for query: {QUERY!r}")
    items = fetch_vam()
    ...
    save_outputs(
        ...
        metadata_filename="vam_metadata.csv",
        coverage_filename="vam_coverage.csv",
        sample_filename="vam_first_10_sample.json",
    )
