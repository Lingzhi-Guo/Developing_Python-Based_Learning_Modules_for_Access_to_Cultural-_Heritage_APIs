...
HARVARD_API_KEY = os.getenv("HARVARD_ART_MUSEUMS_API_KEY") or "fcdc463b-b601-455c-a172-d1a3eab91c91"
OUTPUT_DIR = Path("harvard_output")


def fetch_harvard(query: str = QUERY, limit: int = LIMIT) -> List[Dict[str, Any]]:
    endpoint = "https://api.harvardartmuseums.org/object"
    fields = ",".join([
        "id", "title", "people", "dated", "datebegin", "dateend",
        "culture", "period", "classification", "worktype",
        "primaryimageurl", "copyright", "imagepermissionlevel",
        "url", "accessionyear", "medium", "technique",
        "department", "division",
    ])
    ...
    for idx, doc in enumerate(docs, start=1):
        people = doc.get("people") if isinstance(doc.get("people"), list) else []
        ...
        items.append({
            ...
            "title": join_values(doc.get("title")),
            "creator": creator,
            "date": join_values(first_present(doc.get("dated"), doc.get("datebegin"))),
            "place": join_values(first_present(doc.get("culture"), doc.get("period"))),
            "object_type": join_values(first_present(doc.get("classification"), doc.get("worktype"))),
            "image_url": join_values(doc.get("primaryimageurl")),
            "rights": join_values(doc.get("copyright")),
            "landing_page": join_values(doc.get("url")),
            "source_specific": {
                "accession_year": doc.get("accessionyear"),
                "medium": doc.get("medium"),
                "technique": doc.get("technique"),
                "department": doc.get("department"),
                "division": doc.get("division"),
                "image_permission_level": doc.get("imagepermissionlevel"),
            },
        })
...
def main() -> None:
    print(f"Fetching Harvard Art Museums records for query: {QUERY!r}")
    items = fetch_harvard()
    ...
    save_outputs(
        ...
        metadata_filename="harvard_metadata.csv",
        coverage_filename="harvard_coverage.csv",
        sample_filename="harvard_first_10_sample.json",
    )
